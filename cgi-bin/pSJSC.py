...
class student:    
	...
	#    if not isinstance(news,unicode):
        #   	obj = unicode(news,'gbk')
        #        news=''.join(obj).encode('utf8')
        #   else:
        #        news=''.join(news).encode('utf8')
            filename ='学生信息.txt'
            ...
            stau="登陆成功"
            if not isinstance(stau,unicode): 
                obj = unicode(stau,'utf8')
...
if not isinstance(e,unicode):   #检查编码是否为unicode  
    obj = unicode(e,'utf8')
    e=''.join(obj).encode('utf8')
else:
    e=''.join(e).encode('utf8')
